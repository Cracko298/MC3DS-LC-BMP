...
local function load_and_flip_bmp(path)
    ...
    for y = 0, height - 1 do 
        for x = 0, width - 1 do
            ...
            a = a or 255
            -- Store in sequential order for the swizzler
...
Async.run(function()
    ...
    if Core.Filesystem.fileExists(input) then
        -- This now loads it with the vertical flip
        local px, w, h = load_and_flip_bmp(input) 
        if px then
            ...
            if out_file then
                out_file:write(data)
                out_file:close()
                Core.Debug.log("Converted & Flipped " .. w .. "x" .. h, true)
            end
        end
    else
